...
react(),
    ...
    server: {
      ...
      strictPort: true,
      proxy: {
        // Proxy Tjek/eTilbudsavis API calls to avoid CORS in dev.
        // In production, route /api/tjek through a Supabase Edge Function.
        '/api/tjek': {
          target: 'https://api.etilbudsavis.dk',
          changeOrigin: true,
          rewrite: (path) => path.replace(/^\/api\/tjek/, ''),
        },
      },
      watch: {
        ignored: [
